...
# Module 2 – Data Viz – EX04 Elbow
#
# sternero – 42 Málaga – 2026
#
# Asistente opcional (NO sustituye elbow.*)
# - Estado del entorno (Module 0 .env, Docker, DISPLAY)
# - PostgreSQL: arranque si hace falta
# - SQL de control: nº de compradores (RFM base)
# - Ejecutar elbow.py (ventana o solo PNG)
# - psql, verificar entrega elbow.*, documentación
#
# Uso:
#   cd /ruta/a/data_science_2_data_viz/ex04
#   chmod +x start.sh
#   ./start.sh
#
#   cd ~/sgoinfre/.../piscine_pedago_data_science/data_science_2_data_viz/ex04
...
print_header()
{
    ...
    echo -e "${CYAN}${BOLD}║  Module 2 – Data Viz – EX04 Elbow                          ║${RESET}"
...
status_env()
{
    ...
    if [[ -f "$ELBOW_PY" ]]; then ok "elbow.py presente"; else err "Falta elbow.py"; fi
    if python3 -c "import sklearn" 2>/dev/null; then ok "scikit-learn importable"; else warn "Falta scikit-learn (elbow.py intentará instalar)"; fi
...
run_elbow()
{
    section "📐  Ejecutar elbow.py"
    if [[ ! -f "$ELBOW_PY" ]]; then err "Falta $ELBOW_PY"; return 1; fi
    chmod +x "$ELBOW_PY" 2>/dev/null || true
    echo -e "  ${BOLD}1)${RESET} ventana   ${BOLD}2)${RESET} solo PNG (Agg)"
    local c
    read -r -p "$(echo -e "${YELLOW}[1/2] → ${RESET}")" c
    case "$c" in
        2) ( cd "$SCRIPT_DIR" && MPLBACKEND=Agg python3 "$ELBOW_PY" ) ;;
        *) ( cd "$SCRIPT_DIR" && python3 "$ELBOW_PY" ) ;;
    esac
    local rc=$?
    [[ -f "$SCRIPT_DIR/elbow_method.png" ]] && ok "elbow_method.png"
    [[ $rc -eq 0 ]] && ok "elbow.py OK" || err "código $rc"
}
...
check_delivery()
{
    section "📋  Entrega elbow.*"
    if ls "$SCRIPT_DIR"/elbow.* >/dev/null 2>&1; then
        ok "elbow.* en ex04/"
        ls -la "$SCRIPT_DIR"/elbow.* 2>/dev/null || true
    else
        err "Falta elbow.*"
    fi
}
...
{
    echo
    echo -e "${CYAN}${BOLD}  MENÚ EX04 – Elbow${RESET}"
    echo
    ...
    echo -e "  ${BOLD}4)${RESET}  Ejecutar elbow.py ${WHITE}(recomendado)${RESET}"
    echo -e "  ${BOLD}5)${RESET}  Abrir psql"
    echo -e "  ${BOLD}6)${RESET}  Verificar elbow.*"
    echo -e "  ${BOLD}7)${RESET}  Documentación"
    echo -e "  ${BOLD}p)${RESET}  chmod +x elbow.py / start.sh"
...
{
    local choice
    echo -e "Subject: ${BOLD}Elbow Method${RESET} → nº óptimo de clusters (RFM + KMeans)."
    echo -e "Entrega: ${BOLD}elbow.*${RESET}  ${WHITE}(este menú NO sustituye elbow.*)${RESET}"
    echo
    echo -e "  ${WHITE}Idea:${RESET} resumir cada cliente en Recency / Frequency / Monetary,"
    echo -e "  probar k=1…10 y mirar dónde la curva de inertia se aplana (codo)."
    while true; do
        show_menu
        ...
        case "$choice" in
            ...
            7) show_docs; pause ;;
            p|P) chmod +x "$SCRIPT_DIR/start.sh" "$ELBOW_PY" 2>/dev/null; ok "chmod +x"; pause ;;
...
{
    print_header
    info "Asistente EX04 (Module 2) · RFM + Elbow · fuente: customers"
    echo
